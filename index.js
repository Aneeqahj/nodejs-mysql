...
//  Start a web server on port 3000. Requests go to function handleRequest
...
function handleRequest(request, response) {
  // Page  HTML as on big string, with placeholder "DBCONTENT" for data fro the database
  ...
  if (pathname == "/add") {
    ...
    getContenytsFromDatabase(filter, function (contents) {
...
// Function that is called by the code that handles them / route and retrieves ccontents from the database, applying a LIKE filter if one was supplied

function getContenytsFromDatabase(filter, callback) {
